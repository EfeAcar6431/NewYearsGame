...
function shuffleQuestions() {
  ...
  gameState.shuffledQuestions = categoryQuestions.slice(
    0,
...
function showQuestion() {
  ...
  document.getElementById("q-num").textContent = gameState.currentQuestion + 1;
  document.getElementById("q-total").textContent = gameState.totalQuestions;
  document.getElementById("category-badge").textContent = categories.find(
    (c) => c.id === gameState.selectedCategory
  ).name;
...
function startTimer() {
  ...
  gameState.timerInterval = setInterval(() => {
    gameState.timeLeft--;
    gameState.currentPoints = Math.max(
      100,
      Math.floor(1000 * (gameState.timeLeft / 20))
    );

    updateTimerDisplay();

    // Update circle
    const offset = 283 * (1 - gameState.timeLeft / 20);
...
function updateTimerDisplay() {
  ...
  document.getElementById("points-possible").textContent =
...
function selectAnswer(index) {
  ...
  // Calculate points
  const pointsEarned = isCorrect ? gameState.currentPoints : 0;
  gameState.scores[currentPlayer] += pointsEarned;
...
function timeUp() {
  ...
  setTimeout(() => {
    showResult(false, 0, question.answers[question.correct], currentPlayer);
  }, 1500);
}

function showResult(isCorrect, points, correctAnswer, player) {
  ...
  if (isCorrect) {
    ...
    pointsEarned.style.color = "#ffd700";
  } else {
    resultIcon.textContent = "✗";
    resultIcon.classList.add("wrong");
    resultText.textContent = "Wrong!";
    pointsEarned.textContent = "+0 points";
    pointsEarned.style.color = "#ff6b6b";
  }

  correctAnswerText.textContent = correctAnswer;
  document.getElementById("result-player-name").textContent = player;
  document.getElementById("result-player-score").textContent =
...
function showFinalResults() {
  ...
  if (sortedPlayers.length >= 1) {
    ...
    first.querySelector(".podium-name").textContent = sortedPlayers[0];
    first.querySelector(".podium-score").textContent = `${
      gameState.scores[sortedPlayers[0]]
    } pts`;
  }

  if (sortedPlayers.length >= 2) {
    const second = document.getElementById("second-place");
    second.querySelector(".podium-name").textContent = sortedPlayers[1];
    second.querySelector(".podium-score").textContent = `${
      gameState.scores[sortedPlayers[1]]
    } pts`;
  }

  if (sortedPlayers.length >= 3) {
    const third = document.getElementById("third-place");
    third.querySelector(".podium-name").textContent = sortedPlayers[2];
    third.querySelector(".podium-score").textContent = `${
      gameState.scores[sortedPlayers[2]]
    } pts`;
  }

  // Show all scores
  const allScores = document.getElementById("all-scores");
  allScores.innerHTML = sortedPlayers
    .map(
      (player, index) => `
        <div class="all-scores-item">
            <span>#${index + 1} ${player}</span>
            <span>${gameState.scores[player]} pts</span>
        </div>
    `
    )
    .join("");
...
function resetGame() {
  // Keep same players, start new game
  gameState.currentQuestion = 0;
  gameState.currentPlayerIndex = 0;
